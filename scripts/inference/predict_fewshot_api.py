...
from glasgow_vlm.prompts import structured_plus_fewshot as fewshot_mod


def parse_args():
    parser = argparse.ArgumentParser(description="Run Qwen3-VL-Plus with few-shot examples via DashScope API.")
    ...
    parser.add_argument("--few-shot-json", type=Path, default=fewshot_mod.FEW_SHOT_JSON_PATH, help="Path to few-shot examples JSON.")
...
def main():
    ...
    few_shot_examples = fewshot_mod.load_few_shot_examples(
    ...
    print(f"Prompt module: structured_plus_fewshot")
    ...
    with open(args.output_jsonl, mode, encoding="utf-8") as f:
        ...
        for idx, record in enumerate(samples, start=1):
            ...
            messages = build_fewshot_messages(sample, args.input_mode, args.task, few_shot_examples, fewshot_mod)
