...
Based on train_qwen3_vl_lora.py. The only difference is the default dataset
directory points to the POI-augmented training data built by
build_lora_training_data_poi.py. The training loop and model setup are
identical — the POI context is already baked into the prompt text at data
build time.

Usage:
    # Step 1: build POI-augmented training data
    python scripts/training/build_lora_training_data_poi.py

    # Step 2: train
    python scripts/training/train_qwen3_vl_lora_poi.py
    ...
    python scripts/training/train_qwen3_vl_lora_poi.py \
        --adapter-path outputs/lora_adapters_poi/adapters.safetensors
"""
...
DEFAULT_OUTPUT_PATH = str(ROOT / "outputs" / "lora_adapters_poi" / "adapters.safetensors")


def parse_args():
    p = argparse.ArgumentParser(description="QLoRA fine-tuning for Qwen3-VL-8B (image + POI)")
