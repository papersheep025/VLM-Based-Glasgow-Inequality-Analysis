...
DEFAULT_SATELLITE_PATH = Path("dataset") / "Satellite TIFF" / "glasgow" / "glasgow.tif"
...
DEFAULT_OUTPUT_DIR = Path("dataset") / "aligned_streetview_satellite"


def parse_args():
    parser = argparse.ArgumentParser(
        description="Align Glasgow street-view images to the Glasgow satellite raster."
    )
    parser.add_argument(
        "--satellite-path",
        type=Path,
        default=DEFAULT_SATELLITE_PATH,
        help="Path to the georeferenced satellite TIFF.",
    )
    parser.add_argument(
        "--metadata-path",
        type=Path,
    ...
    parser.add_argument(
        ...
        choices=("png", "jpg"),
        default="png",
        help="Image format for saved satellite patches.",
    )
    parser.add_argument(
        "--limit",
        type=int,
    ...
    parser.add_argument(
        "--skip-patches",
        action="store_true",
        help="Only compute the alignment table, without saving satellite patches.",
    )
    parser.add_argument(
        "--overwrite",
        action="store_true",
...
def main():
    ...
    patches_dir = output_dir / "satellite_patches"
    ...
    with rasterio.open(args.satellite_path) as dataset:
        if dataset.crs is None:
            raise ValueError(f"Satellite raster has no CRS: {args.satellite_path}")
        ...
        for _, row in metadata.iterrows():
            ...
            if inside_raster and not args.skip_patches:
                patch_name = f"{Path(image_name).stem}_satellite_{patch_size}.{args.patch_format}"
                patch_path = patches_dir / patch_name
                patch_relpath = patch_path.relative_to(output_dir).as_posix()
    ...
    aligned_csv_path = output_dir / "streetview_satellite_alignment.csv"
    aligned_json_path = output_dir / "streetview_satellite_alignment.json"
    ...
    summary = {
        "satellite_path": str(args.satellite_path),
    ...
    if not args.skip_patches:
        print(f"Satellite patches saved under: {patches_dir}")
