...
SYSTEM_PROMPT = (
    ...
    "Think step-by-step and show your full reasoning in the output. "
    "Return JSON only."
)
...
_INTERMEDIATE_RULES = (
    ...
    "For each, provide the supporting evidence phrases and a score 1–10 (1=poorest quality, 10=best quality), "
    "plus a brief interpretation. "
    "Variables: "
    ...
    "If evidence conflicts across modalities, note the conflict in the interpretation. "
    "If evidence is weak for a variable, use a mid-range score (4–7) and note uncertainty. "
)

_DIMENSION_RULES = (
    "STEP 3 — Score each SIMD dimension 1–10 (1=most deprived, 10=least deprived), "
    "drawing only on Step 1 evidence and Step 2 variables. "
    "Provide a reasoning string for each score. "
    ...
    "education — image evidence is weak; use school/educational facility visibility and environmental quality as weak proxies only; if uncertain, default to 5 and note low confidence. "
...
_FORMAT_RULES = (
    "Output one valid JSON object only. All fields must be present. "
    "The JSON must contain: "
    "a 'reasoning' object with keys: "
    "  'evidence' (object with keys streetview, satellite, nightlight — each a list of 3–5 short phrases under 8 words), "
    "  'intermediate_variables' (object with one entry per variable, each having keys: score (int 1–10), support (list of phrase strings), interpretation (string)), "
    "  'dimension_reasoning' (object with one string entry per dimension: income, employment, health, education, housing, access, crime); "
    "and seven score keys: income, employment, health, education, housing, access, crime (each an integer 1–10); "
    "and overall (a float). "
    "Do not omit any key. Nightlight evidence must describe only light intensity and spatial distribution — no semantic object labels. "
    "Before outputting, verify the JSON is complete and valid."
)

_SCHEMA_EXAMPLE = json.dumps({
    "reasoning": {
        "evidence": {
            "streetview": ["cracked pavements", "graffiti on walls", "few trees"],
            "satellite": ["dense terraced housing", "narrow road grid", "little green space"],
            "nightlight": ["dim uniform glow", "patchy coverage at edges", "concentrated centre"]
        },
        "intermediate_variables": {
            "built_environment_quality": {"score": 4, "support": ["cracked pavements", "dense terraced housing"], "interpretation": "Poor maintenance and high density suggest below-average quality."},
            "infrastructure_adequacy": {"score": 5, "support": ["narrow road grid"], "interpretation": "Roads present but narrow and limited."},
            "residential_environment_quality": {"score": 4, "support": ["cracked pavements", "graffiti on walls"], "interpretation": "Visible neglect and disorder."},
            "connectivity_accessibility": {"score": 5, "support": ["narrow road grid", "concentrated centre"], "interpretation": "Moderate connectivity, centrally lit area."},
            "commercial_economic_activity": {"score": 3, "support": ["graffiti on walls"], "interpretation": "Low visible commercial presence."},
            "night_time_activity_coverage": {"score": 4, "support": ["dim uniform glow", "patchy coverage at edges"], "interpretation": "Dim lighting with edge darkness."},
            "open_space_greenery": {"score": 3, "support": ["few trees", "little green space"], "interpretation": "Very limited green space."},
            "spatial_marginality": {"score": 6, "support": ["concentrated centre", "narrow road grid"], "interpretation": "Centrally located, not isolated."}
        },
        "dimension_reasoning": {
            "income": "Residential decline and vacancy cues suggest below-median income proxy; cautious inference.",
            "employment": "Low commercial activity and limited transport links indicate reduced employment access.",
            "health": "Minimal green space and poor walkability environment proxy for health risk.",
            "education": "Insufficient image evidence; mid-range score assigned with low confidence.",
            "housing": "Dense, poorly maintained terraced housing visible in both street-view and satellite.",
            "access": "Central location but narrow road capacity limits connectivity.",
            "crime": "Graffiti and disorder cues suggest elevated environmental risk for crime."
        }
    },
    "income": 4,
    "employment": 4,
